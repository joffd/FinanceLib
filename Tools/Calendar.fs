...
module Date =
    ...
    type public Tenor = { 
        ...
        Days: int } with
            override this.ToString() =
                sprintf "Year(s): %i | Month(s): %i | Day(s): %i" 
    ...
    let public stdWeekendSet = [DayOfWeek.Saturday; DayOfWeek.Sunday] |> Set.ofList
    
    /// Check Date (and not DateTime) equality
    let public checkDateEquality (d1: DateTime) (d2: DateTime) =
        d1.Date = d2.Date

    let public compateDateTo (d1: DateTime) (d2: DateTime) =
    ...
    let public businessDaysBetween calendar (startDate: System.DateTime) (endDate: System.DateTime) =
        let rec builder currDate acc =
            if currDate < startDate then acc
                ...
                if isBusinessDay calendar currDate then builder (dayBefore currDate) (currDate :: acc)
    ...
    let rec public rollBusinessDay rule calendar date =
        if isBusinessDay calendar date then date
                ...
                | RollRule.ModifiedFollowing    ->
                        let next = rollBusinessDay RollRule.Following calendar date
                        if next.Month <> date.Month then rollBusinessDay RollRule.Previous calendar date else next
                    | RollRule.ModifiedPrevious    ->
                        let prev = rollBusinessDay RollRule.Previous calendar date
                        if prev.Month <> date.Month then rollBusinessDay RollRule.Following calendar date else prev
        ...
        match n with 
            ...
            | x -> 
                ...
                | RollRule.ModifiedFollowing    ->  
                    ...
                    let next = rollBusinessDayBy (x - 1) RollRule.Following calendar date
                    // Roll the last day ModifiedFollowing
                    let final = rollBusinessDay RollRule.Following calendar (dayAfter next)
                ...
                | RollRule.ModifiedPrevious     ->  
                    ...
                    let next = rollBusinessDayBy (x - 1) RollRule.Previous calendar date
                         
                    // Roll the last day ModifiedPrevious
                    let final = rollBusinessDay RollRule.Previous calendar (dayAfter next)
                    if final.Month <> next.Month 
    ...
    let combineCalendars (calendars: list<Calendar>) =
        let rec intersect (res: Set<'a>) list =
            ...
            | [] -> res
            | h::t ->
                intersect (Set.intersect h res) t
