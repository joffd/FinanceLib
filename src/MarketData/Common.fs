...
module Common =
    ...
    let fromContinousRateToDF (f: DateTime -> DateTime -> float) =
        fun (dateNow: DateTime) (dateLater: DateTime) ->
            Math.Exp
                (-1.0
                 * dateLater.Subtract(dateNow).TotalDays
                 / 365.25
                 * (f dateNow dateLater))
    ...
    let fromDFToContinousRate (f: DateTime -> DateTime -> float) =
        fun (dateNow: DateTime) (dateLater: DateTime) ->
            -1.0
            * Math.Log(f dateNow dateLater)
            / (dateLater.Subtract(dateNow).TotalDays / 365.25)
    ...
    let rates =
        [ 0.0 .. 0.001 .. 0.01 ] |> List.map ((+) 0.02)

    let t1 = List.zip nextDays rates |> Array.ofList
    ...
    let df = fromContinousRateToDF countinious
